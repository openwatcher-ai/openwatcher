<template>
  <button
    ...
    class="floating-orb"
    :class="[status, { dragging, unavailable: slides.length === 0 }]"
...
const slides = computed(() => {
  ...
  return [
    { key: '5h', label: '5h', data: props.quota?.fiveHour },
    { key: '7d', label: '7d', data: props.quota?.weekly },
...
const ariaLabel = computed(() => active.value.data
  ? `展开用量概览，当前显示 ${active.value.label} 剩余额度 ${Math.round(active.value.data.remainingPercent)}%`
  : '展开用量概览，额度数据不可用')
